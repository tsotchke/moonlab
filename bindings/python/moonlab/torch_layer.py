...
class ParameterShiftGradient(_torch.autograd.Function):
    ...
    last_backend_trace: Dict[str, Any] = {}
    ...
    def backward(ctx, grad_output):
        ...
        for i in range(inputs.numel()):
            ...
            inputs_plus.reshape(-1)[i] += eps
            outputs_plus = circuit_fn(inputs_plus, params)
            measure_plus = measurement_fn(outputs_plus)

            inputs_minus = inputs.clone()
            inputs_minus.reshape(-1)[i] -= eps
            outputs_minus = circuit_fn(inputs_minus, params)
            measure_minus = measurement_fn(outputs_minus)

            # Finite difference gradient
            flat_grad_inputs[i] = ((measure_plus - measure_minus) * grad_output).sum() / (2.0 * eps)
...
class QuantumLayer(nn.Module):
    ...
    def _execute_batch(self, x: _torch.Tensor, params: _torch.Tensor) -> _torch.Tensor:
        ...
        """
        was_vector = x.dim() == 1
        x_batch = x.unsqueeze(0) if was_vector else x
        batch_size = x_batch.shape[0]
        outputs = []
        self._last_backend_trace = _make_torch_backend_trace("QuantumLayer", "forward")
        ...
        for batch_idx in range(batch_size):
            ...
            # Encode input data
            input_vec = x_batch[batch_idx]
            self.encode_data(input_vec, state)

            # Apply variational circuit
            self.apply_variational_circuit(state, params)
    ...
    def forward(self, x: _torch.Tensor) -> _torch.Tensor:
        ...
        """
        return ParameterShiftGradient.apply(
            x,
            self.params,
            self._execute_batch,
            _identity_measurement,
            self.num_qubits
        )
