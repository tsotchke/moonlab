...
from .core import QuantumState, _lib_path

logger = logging.getLogger(__name__)


_LAST_BACKEND_TRACE: Dict[str, Any] = {}


def moonlab_torch_backend_probe(owner: str = "moonlab_torch_backend_probe",
                                operation: str = "probe",
                                reference: Optional[torch.Tensor] = None) -> Dict[str, Any]:
    """Report the native Moonlab runtime used by PyTorch integration paths."""
    if reference is not None:
        torch_device = reference.device.type
    ...
    return {
        ...
        "fallback_intentional": torch_device != "cpu",
        "fallback_reason": "Moonlab quantum kernels execute through the loaded ctypes library",
        "missing_symbols": [],
    }


def _record_backend_trace(owner: str,
                          operation: str = "runtime",
                          reference: Optional[torch.Tensor] = None) -> Dict[str, Any]:
    global _LAST_BACKEND_TRACE
    _LAST_BACKEND_TRACE = moonlab_torch_backend_probe(owner, operation, reference)
    return dict(_LAST_BACKEND_TRACE)


def moonlab_torch_last_backend_trace() -> Dict[str, Any]:
    if not _LAST_BACKEND_TRACE:
        return moonlab_torch_backend_probe()
    return dict(_LAST_BACKEND_TRACE)


def _measurement_tensor(values, reference: torch.Tensor) -> torch.Tensor:
    _record_backend_trace("_measurement_tensor", "measurement_tensor", reference)
...
class ParameterShiftGradient(torch.autograd.Function):
    ...
    """
    last_backend_trace = moonlab_torch_backend_probe(
        "ParameterShiftGradient", "uninitialized"
    )
    ...
    def backward(ctx, grad_output):
        ...
        ParameterShiftGradient.last_backend_trace = _record_backend_trace(
            "ParameterShiftGradient.backward", "backward", grad_output
        )
...
class QuantumConv1D(nn.Module):
    ...
    def __init__(
        ...
        self.last_backend_trace = moonlab_torch_backend_probe(
            "QuantumConv1D", "uninitialized"
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        self.last_backend_trace = _record_backend_trace(
            "QuantumConv1D.forward", "forward", result
        )
        return result
...
class QuantumPooling(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.method == 'measure':
            ...
            self.last_backend_trace = _record_backend_trace(
                "QuantumPooling.forward", "forward", x
            )
            return x
            ...
            if len(x.shape) == 3:
                ...
                result = torch.stack(outputs)
                self.last_backend_trace = _record_backend_trace(
                    "QuantumPooling.forward", "forward", result
                )
                return result
            else:
                # Direct feature pooling
                result = self.quantum_pool(x)
                self.last_backend_trace = _record_backend_trace(
                    "QuantumPooling.forward", "forward", result
                )
                return result
            ...
            # Partial-trace-inspired pooling: return per-window density blocks.
            if len(x.shape) == 3:
                batch_size, seq_len, features = x.shape
                new_seq_len = seq_len // self.pool_size
                x = x[:, :new_seq_len * self.pool_size, :]
                windows = x.view(batch_size, new_seq_len, self.pool_size, features)
                result = windows.conj().transpose(-2, -1) @ windows
            elif len(x.shape) == 2:
                batch_size, features = x.shape
                new_features = features // self.pool_size
                x = x[:, :new_features * self.pool_size]
                windows = x.view(batch_size, new_features, self.pool_size)
                result = windows.unsqueeze(-1) * windows.conj().unsqueeze(-2)
            else:
                raise ValueError("trace pooling expects a 2D or 3D tensor")
            self.last_backend_trace = _record_backend_trace(
                "QuantumPooling.forward", "forward", result
            )
            return result
