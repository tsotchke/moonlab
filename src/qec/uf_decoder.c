    ...
    for (int improved = 1; improved && ncl > 1;) {
        ...
        for (unsigned i = 0; i + 1 < ncl; i++) {
            ...
            for (unsigned j = i + 1; j < ncl; j++) {
                ...
                /* Lossless prune, in the context metric: a joint solution
                 * beats the separate optima only through a cross pairing,
                 * and any cross pairing (a,b) can be replaced by two
                 * boundary assignments at no extra cost unless
                 * eff(a,b) < beff(a) + beff(b).  If no cross pair satisfies
                 * that, the joint optimum decomposes back into separate
                 * solutions and the 2^k DP cannot gain.  Expressed through
                 * the distance context, so it stays exact for pass 2's
                 * boosted tables too.  O(ki*kj) lookups vs the DP. */
                {
                    int useful = 0;
                    for (unsigned qa = i; qa != UINT32_MAX && !useful;
                         qa = s->cl_next[qa])
                      for (uint32_t ta = s->cl_off[qa];
                           ta < s->cl_off[qa + 1] && !useful; ta++) {
                        uint32_t a = ids ? ids[ta] : ta;
                        double ba = c->bd[a] - c->blm[a];
                        for (unsigned qb = j; qb != UINT32_MAX && !useful;
                             qb = s->cl_next[qb])
                          for (uint32_t tb = s->cl_off[qb];
                               tb < s->cl_off[qb + 1]; tb++) {
                            uint32_t b = ids ? ids[tb] : tb;
                            size_t ab = (size_t)a * c->stride + b;
                            if (c->dist[ab] - c->lm[ab] <
                                ba + c->bd[b] - c->blm[b]) {
                                useful = 1; break;
                            }
                          }
