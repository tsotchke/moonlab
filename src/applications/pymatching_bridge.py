...
def _build_toric_matching_graph(d):
    ...
    Each Z-vertex stabiliser is a node; horizontal edges (data qubits)
    connect vertex (a, b) to (a, b+1); vertical edges connect (a, b)
    to (a+1, b).  All-1 weights -- minimum-weight matching collapses
    to minimum-weight by edge count, which on a uniform torus equals
    the L1 geodesic moonlab's in-tree MWPM_EXACT computes."""
    import pymatching
    m = pymatching.Matching()
    n_vertices = d * d
    # Add a fault id per edge (data qubit).  Horizontal edges first.
    fault_id = 0
    for a in range(d):
        for b in range(d):
            u = a * d + b
            ...
            fault_id += 1
    # Vertical edges: vertex (a, b) -- vertex ((a+1) % d, b).  Fault id
    # offset = d*d so the second half of the correction array matches
    # moonlab's horizontal-then-vertical layout.
    for a in range(d):
        for b in range(d):
            u = a * d + b
            v = ((a + 1) % d) * d + b
